...
{
    class ArgsNameParam : INameParam
    {
        ...
        {
            if (_notLongerThan.HasValue) yield return new KeyValuePair<RuleType, object>(RuleType.ParamNameNotLongerThan, _notLongerThan);
            if (_notShorterThan.HasValue) yield return new KeyValuePair<RuleType, object>(RuleType.ParamNameNotShorterThan, _notShorterThan);
            if (_startsWith.Any()) yield return new KeyValuePair<RuleType, object>(RuleType.ParamNameStartsWith, _startsWith);
            if (_startWithLowerCase.HasValue) yield return new KeyValuePair<RuleType, object>(RuleType.ParamNameStartsWithLowerCase, _startWithLowerCase);
            if (_notStartsWith.Any()) yield return new KeyValuePair<RuleType, object>(RuleType.ParamNameNotStartsWith, _notStartsWith);
            if (_matchRegex.Any()) yield return new KeyValuePair<RuleType, object>(RuleType.ParamNameMatchRegex, _matchRegex);
            if (_endsWith.Any()) yield return new KeyValuePair<RuleType, object>(RuleType.ParamNameEndsWith, _matchRegex);
            if (_notEndsWith.Any()) yield return new KeyValuePair<RuleType, object>(RuleType.ParamNameNotEndsWith, _matchRegex);
        }
    }
}
